...
|:--:|:---|:---|:---|
| 1 | **Edit** | open `01-international-financial-system/01-international-financial-system.qmd` | The only file with slide content. RStudio, Positron or any text editor. |
| 2 | **Write live** | `quarto preview 01-international-financial-system/01-international-financial-system.qmd` | Opens the deck in your browser and re-renders on every save. Leave it running while you work. `Ctrl+C` to stop. |
| 3 | **Build the slides** | `quarto render 01-international-financial-system/01-international-financial-system.qmd --to revealjs` | Produces `_site/01-international-financial-system/01-international-financial-system.html`. This is the file you present from. |
| 4 | **Build the handout** | `quarto render 01-international-financial-system/01-international-financial-system.qmd --to beamer` | Produces the PDF. Needs LaTeX — run `quarto install tinytex` once if it fails. |
...
| 6 | **Present** | open `_site/01-international-financial-system/01-international-financial-system.html` | Double-click it. Works offline except the two YouTube embeds and the poll. |
...
|:---|:---|:---|
| `<session>/<session>.qmd` | All slide content: text, chunks, tables | Always — this is the deck |
| `_theme/theme.scss` | Colours, type sizes, callout boxes, dividers | You want it to *look* different |
| `_R/setup.R` | Chart palette, ggplot theme, the FRED helper | You want the *figures* to look different |
...
| `<session>/data/*.csv` | The tables in Problems 1.1 and 1.2 | The numbers change |
